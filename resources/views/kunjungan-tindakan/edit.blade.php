...
@section('title', 'Edit Tindakan Kunjungan')
    ...
    <nav class="flex mb-6" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    <i class="fas fa-chevron-right text-gray-400 mx-2"></i>
                    <span class="text-sm font-medium text-gray-500">Edit Tindakan Kunjungan</span>
                </div>
            </li>
        </ol>
    </nav>
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-900">Edit Tindakan Kunjungan</h1>
        </div>

        <div class="bg-white shadow-md rounded-lg p-6">
            <form action="{{ route('kunjungan-tindakan.update', $kunjunganTindakan) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <select name="kunjungan_id" id="kunjungan_id" 
                            ...
                            @foreach($kunjungans as $kunjungan)
                                <option value="{{ $kunjungan->id }}" 
                                    {{ (old('kunjungan_id', $kunjunganTindakan->kunjungan_id) == $kunjungan->id) ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="tindakan_id" id="tindakan_id" 
                            ...
                            @foreach($tindakans as $tindakan)
                                <option value="{{ $tindakan->id }}" data-tarif="{{ $tindakan->tarif }}" 
                                    {{ (old('tindakan_id', $kunjunganTindakan->tindakan_id) == $tindakan->id) ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="number" 
                               name="jumlah" 
                               id="jumlah"
                               value="{{ old('jumlah', $kunjunganTindakan->jumlah) }}"
                               min="1"
                    ...
                    <div>
                        ...
                        <input type="number" 
                               name="tarif" 
                               id="tarif"
                               value="{{ old('tarif', $kunjunganTindakan->tarif) }}"
                               step="0.01"
                               min="0"
                ...
                <div class="mt-6">
                    ...
                    <input type="text" 
                           ...
                           readonly
                           value="Rp {{ number_format($kunjunganTindakan->subtotal, 0, ',', '.') }}">
                </div>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="catatan" 
                              ...
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('catatan') border-red-500 @enderror">{{ old('catatan', $kunjunganTindakan->catatan) }}</textarea>
                    @error('catatan')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mt-8 flex justify-end space-x-4">
                    <a href="{{ route('kunjungan-tindakan.show', $kunjunganTindakan) }}" 
