    ...
    <nav class="flex mb-6" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    <i class="fas fa-chevron-right text-gray-400 mx-2"></i>
                    <span class="text-sm font-medium text-gray-500">Edit Kunjungan</span>
                </div>
            </li>
        </ol>
    </nav>
            ...
            <form action="{{ route('kunjungan.update', $kunjungan) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <select name="pasien_id" id="pasien_id" 
                            ...
                            @foreach($pasiens as $pasien)
                                <option value="{{ $pasien->id }}" 
                                    {{ (old('pasien_id', $kunjungan->pasien_id) == $pasien->id) ? 'selected' : '' }}>
                        ...
                        <select name="dokter_id" id="dokter_id" 
                            ...
                            @foreach($dokters as $dokter)
                                <option value="{{ $dokter->id }}" 
                                    {{ (old('dokter_id', $kunjungan->dokter_id) == $dokter->id) ? 'selected' : '' }}>
                                    {{ $dokter->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" 
                               ...
                               value="{{ old('tanggal_kunjungan', $kunjungan->tanggal_kunjungan->format('Y-m-d\TH:i')) }}"
                    ...
                    <div>
                        ...
                        <select name="jenis_kunjungan" id="jenis_kunjungan" 
                            ...
                            <option value="umum" {{ old('jenis_kunjungan', $kunjungan->jenis_kunjungan) === 'umum' ? 'selected' : '' }}>Umum</option>
                            <option value="rujukan" {{ old('jenis_kunjungan', $kunjungan->jenis_kunjungan) === 'rujukan' ? 'selected' : '' }}>Rujukan</option>
                            <option value="kontrol" {{ old('jenis_kunjungan', $kunjungan->jenis_kunjungan) === 'kontrol' ? 'selected' : '' }}>Kontrol</option>
                            <option value="rutin" {{ old('jenis_kunjungan', $kunjungan->jenis_kunjungan) === 'rutin' ? 'selected' : '' }}>Rutin</option>
                            <option value="vaksinasi" {{ old('jenis_kunjungan', $kunjungan->jenis_kunjungan) === 'vaksinasi' ? 'selected' : '' }}>Vaksinasi</option>
                            <option value="darurat" {{ old('jenis_kunjungan', $kunjungan->jenis_kunjungan) === 'darurat' ? 'selected' : '' }}>Darurat</option>
                        </select>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="keluhan" 
                              ...
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('keluhan') border-red-500 @enderror">{{ old('keluhan', $kunjungan->keluhan) }}</textarea>
                    @error('keluhan')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="diagnosa" 
                              ...
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 @error('diagnosa') border-red-500 @enderror">{{ old('diagnosa', $kunjungan->diagnosa) }}</textarea>
                    @error('diagnosa')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
